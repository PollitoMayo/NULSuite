...
import {
  TriggerEvent, Subject, EffectCategory, StatusEffect, ConditionEffect, Barrier,
  TRIGGER_EVENT_LABELS, SUBJECT_LABELS, EFFECT_VALUE_LABELS, EFFECTS_BY_CATEGORY,
  TRIGGERS_WITH_SUBJECT, MECHANIC_PARAM_TYPE, EffectParamType,
    ...
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <fieldset style={s.fieldset}>
        ...
        {triggers.map((tr, i) => {
          ...
          const needsTypeParam  = tr.event === TriggerEvent.ON_MOVE_TYPE;
          return (
            <div key={i} style={{ marginBottom: 12, paddingBottom: 12, borderBottom: i < triggers.length - 1 ? "1px solid var(--border)" : "none" }}>
              ...
              {needsTypeParam && (
                <div style={{ marginTop: 8 }}>
                  <label style={s.smLabel}>Tipo de movimiento</label>
                  <select value={tr.param} onChange={(e) => setTrigger(i, "param", e.target.value)}>
                    <option value="">— Tipo —</option>
                    {PokemonType.all().map((t) => (
                      <option key={t.codeName} value={t.codeName}>{t.displayName}</option>
